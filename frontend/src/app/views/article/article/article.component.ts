...
export class ArticleComponent implements OnInit {
  ...
  //reactionsArticleComments: ReactionsType[] = [];
  ...
  submitForm(writeComment: NgForm) {
    if (writeComment.valid && this.article.id) {
      this.commentsService.sendNewComment(this.commentForm.text, this.article.id)
        .subscribe({
          next: (data) => {
            if (data.error) {
  ...
  downloadComments() {
    this.isLoading = true;
    this.commentsService.getComments(this.commentCountOnPage, this.article.id)
      .subscribe(data => {
        if ((data as DefaultResponseType).error) {
